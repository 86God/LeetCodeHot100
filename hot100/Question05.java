...
///////////////////////////////////////////////////////////////////////////////////////////
	...
	/*
	 * 方法：中心扩展算法 我们发现回文字符串有个特点，就是左右两边互为镜像，我们可以利用这一特点，
	 * 先找到中心点，然后分别向左右两边检测，直到遍历完所有的中心点，找到最大的回文字符字串 中心点共有2n-1个，其中有两种情况，
	 * 一种子串是奇数个字符，中心点为中间的数， 另一种是子串有偶数个字符，中间点就是中间两个数间的间隙 所以，n个数，n-1个间隙，总共2n-1个中心点
	 * 时间复杂度:O(n^2) 
	 * 空间复杂度:O(1)
	 */
	...
	public static int nowLongest(String s, int left, int right) {
...
///////////////////////////////////////////////////////////////////////////////////////////
	/*
	 * 方法：最长公共子串 算法：我们可以根据回文串的特点，左右对称的特点，我们可以再定义一个字符串存放原字符串的倒置字符串reverse,
	 * 然后比较两个字符串找到最长公共子串，该公共子串就是该字符串的最长回文串。
	 * 但是，会有特殊情况，比如abc123cba，他的倒置字符串是abc123cba，最长公共子串为abc（或cba），
	 * 我们会发现并不是正确答案，所以我们的算法还需要加个判断条件， 我们需要判断倒置前的下标是否和倒置后的下标匹配，及是否是同一位置的元素
	 * 找到一个字符倒置前的下标很简单，比如该字符下标为j，那么before = length - 1 - j;
	 * 比如我们用i指向原字符串子串的末尾字符，用j指向倒置字符串子串的末尾字符， j倒置前的位置下标before_j = length - 1 - j；
	 * 如果before_j + maxson.len -1 == i，那么说明它就是我们要找的回文串。
	 * 
	 * 
	 * 分析完思路，我们再来看看最长公共子串如何求解？ 这个有很多种方法，我们就有动态规划法解决。
	 * 思路：申请一个二维的数组arr[len][len]，把每个元素初始化为0
	 * 写个双重循环遍历二维数组，其中i表示原字符串s的下标，j表示倒置字符串reverse的下标.
	 * 然后判断对应的字符是否相等，及判断s.charAt(i)==reverse.charAt(j) 相等的话 arr[i][j] =
	 * arr[i-1][j-1] + 1。 其中i=0或j=0的情况单独分析，如果字符相等的话直接赋值arr[i][j]=1.
	 * 最后arr[i][j]里面就是保存着公共子串的长度 该公共子串就是以i结尾的，我们用i-arr[i][j]-1就可以找到子串开始的下标，
	 * 再截取原字符串就可以得到最长公共子序列。
	 * 
	 * 
	 *时间复杂度：O(n^2)	两层for循环
...
///////////////////////////////////////////////////////////////////////////////////////////	
	...
	public static void main(String[] args) {
		String string = "aabbccbbaer";
		System.out.println(longestPalindrome4(string));
	}
}
